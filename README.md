...
8. Обучить: `whisper.py --data-dirs common_voice common_voice_elang
   ...
   **Важно:** `--max-steps` (по умолчанию 10000) нужно оценивать относительно
   реального размера объединённого корпуса, а не оставлять как есть — при
   ~175k примеров и batch size 4 это лишь ~0.23 эпохи. Один такой прогон уже
   был доведён до конца (не в этом окружении) и заметно проиграл публичному
   `AigizK/whisper-medium-ba`; разбор причин и результаты — REPORT.md, раздел 11.
...
- `trainer_state.json` — лог обучения `whisper.py` (`Seq2SeqTrainer`) с
  другой машины, где реально прошёл прогон до `whisper-ba` (сам
  чекпоинт/каталог `whisper-ba/` в этом чекауте отсутствует). Разбор —
  REPORT.md, раздел 11.
- `common_voice_pairs_poems_aigizk.csv`, `common_voice_pairs_poems_whisper_ba.csv`
  (и `*_cleaned.csv`, `common_voice_pairs_poems_comparison.csv`) — результаты
  сверки `whisper-ba` и `AigizK/whisper-medium-ba` через
  `transcribe_evaluate_asr.py` на `common_voice_pairs_poems`. Итоги —
  REPORT.md, раздел 11.
