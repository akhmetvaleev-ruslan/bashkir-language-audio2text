...
pairs_to_common_voice.py / eaf_to_common_voice.py) with a Whisper checkpoint
fine-tuned by whisper.py (a local --output-dir, e.g. "whisper-ba") - or any
Whisper checkpoint on the HF Hub - then score the hypotheses against the
corpus' ground-truth `sentence` column.

Unlike transcribe_evaluate_asr.py (which also accepts CTC encoders such as
Wav2Vec2/Wav2Vec2-BERT/HuBERT and auto-detects the model type), this script
only ever loads WhisperForConditionalGeneration + WhisperProcessor, since
that's the only architecture whisper.py produces.

--language/--task default to None (use the checkpoint's own
generation_config, e.g. the Bashkir/transcribe values whisper.py bakes in
via model.generation_config at save time) rather than being forced, since
overriding via processor.get_decoder_prompt_ids() bypasses that saved config.

Requires:
  pip install torch torchaudio transformers accelerate jiwer soundfile pandas tqdm

Usage:
  python transcribe_evaluate_whisper.py --data-dir common_voice_pairs_poems \
      --model-id whisper-ba --batch-size 32
"""
...
def transcribe_batch(model, processor, audios, target_sr, device, autocast_ctx, language, task, max_new_tokens):
...
def transcribe_corpus(
    df: pd.DataFrame, clips_dir: Path, model, processor, device: str,
...
def main():
    parser = argparse.ArgumentParser(description="Transcribe and evaluate a Common Voice-style corpus with a whisper.py checkpoint")
    parser.add_argument("--data-dir", default="common_voice_pairs_poems")
    parser.add_argument("--tsv-name", default="train.tsv")
    parser.add_argument("--model-id", default="whisper-ba", help="HF Hub id or local checkpoint directory (e.g. a whisper.py --output-dir)")
    ...
    parser.add_argument(
        ...
        help="Force decoding language (e.g. 'Bashkir'). Leave unset to use the checkpoint's own "
             "generation_config (whisper.py bakes in LANGUAGE/TASK there at save time).",
    )
    parser.add_argument(
        "--task", default=None,
        help="'transcribe' or 'translate'. Leave unset to use the checkpoint's own generation_config.",
    )
    ...
    results_df = transcribe_corpus(
        df, clips_dir, model, processor, args.device,
